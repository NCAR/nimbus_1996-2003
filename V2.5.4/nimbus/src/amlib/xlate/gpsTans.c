...
void xlgalt(RAWTBL *varp, void *input, NR_TYPE *output)
{
  ...
  posTime = gps->postime - (int)gps->postime;
  posLag = (posTime - 0.5) * 1000;

  *output = (NR_TYPE)gps->galt;
...
void xlgvew(RAWTBL *varp, void *input, NR_TYPE *output)
{
  ...
  velTime = gps->postime - (int)gps->postime;
  velLag = (velTime - 0.5) * 1000;

  *output = (NR_TYPE)gps->veleast;
