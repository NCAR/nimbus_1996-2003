...
/*  Following is left-over old code in case there are problems
    CX_2 = (GetDefaultsValue("CX_2", varp->name));
    printf ("CX_2 Values set to %f, %f, %f, %f in AMLIB function lymanInit.\n", CX_1);
  if (CX_2[0] == 0.0 && CX_2[1] == 0.0 && CX_2[2] == 0.0 && CX_2[3] == 0.0)
  {
    CX_2 = cx_2;
    sprintf(buffer, "Values set to %f, %f, %f, %f in AMLIB function lymanInit.\n", CX_2);
    LogMessage(buffer);
  }
 */
