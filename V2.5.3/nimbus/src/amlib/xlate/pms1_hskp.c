...
#define SecondPoly2(x, v)	((v->cof[0] / v->SampleRate) + \
				 (x * v->SampleRate * v->cof[1]) + \
				 (x * v->cof[2]))
...
void xlpms16_hskp4(RAWTBL *varp, void *input, NR_TYPE *np)
{
  ...
  for (i = 0, np[0] = 0.0; i < varp->SampleRate; ++i)
    np[i] = (NR_TYPE)SecondPoly2((NR_TYPE)ntohs(p[i].aux[4]), varp);
...
void xlpms16_hskp5(RAWTBL *varp, void *input, NR_TYPE *np)
{
  ...
  for (i = 0; i < varp->SampleRate; ++i)
    np[i] = (NR_TYPE)SecondPoly2((NR_TYPE)ntohs(p[i].aux[5]), varp);
...
void xlpms16_hskp6(RAWTBL *varp, void *input, NR_TYPE *np)
{
  ...
  for (i = 0; i < varp->SampleRate; ++i)
    np[i] = (NR_TYPE)SecondPoly2((NR_TYPE)ntohs(p[i].aux[6]), varp);
...
void xlpms16_hskp7(RAWTBL *varp, void *input, NR_TYPE *np)
{
  ...
  for (i = 0; i < varp->SampleRate; ++i)
    np[i] = (NR_TYPE)SecondPoly2((NR_TYPE)ntohs(p[i].aux[7]), varp);
