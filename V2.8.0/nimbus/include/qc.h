...
extern "C" int qc_rt_init(void);   /* (not using 'boolean' to avoid conflict)*/
extern "C" void qc_rt_dumpstate(void);  /* (with chris) */
extern "C" int qc_rt_exec(void);
extern "C" void qc_rt_cleanup(void);

extern "C" int qc_pp_init(void);
extern "C" int qc_pp_exec(void);
extern "C" void qc_pp_cleanup(void);
