...
static PMS	pms1v2_probes[] =
	...
	Y200_STR,	Y200NAMES,	Y200,	PROBE_200Y,	0,
	X260_STR,	X260NAMES,	X260,	PROBE_260X,	0,
	MASP_STR,	NULL,		MASP,	PROBE_MASP,	0,
	NULL,		NULL,		0,	0,		0
	};

static PMS	pms1v3_probes[] =
	{
	SPP100_STR,	NULL,		FSSP,	PROBE_FSSP,	0,
	SPP200_STR,	NULL,		ASAS,	PROBE_PCASP,	0,
	SPP300_STR,	NULL,		F300,	PROBE_F300,	0,
	NULL,		NULL,		0,	0,		0
	};

static PMS	pms2d_probes[] =
	{
	P2D_C_STR,	TWODNAMES,	0,	PROBE_PMS2D,	0,
	P2D_P_STR,	TWODNAMES,	0,	PROBE_PMS2D,	0,
...
static int	InertialSystemCount, GPScount, twoDcnt;
...
static void add_file_to_RAWTBL(), add_file_to_DERTBL(), initHDR(), initSDI(),
	initHoneywell(), initOphir3(), initPMS1D(), initPMS1Dv2(), initTans(),
	initLitton51(), add_derived_names(), initPMS2D(), initPMS2Dhouse(),
	initGreyHouse(), initMASP(), initPMS1Dv3();
...
int DecodeHeader(char header_file[])
{
  ...
  nsdi = nraw = nderive = InertialSystemCount = GPScount = twoDcnt = 0;
  ...
  */
  ...
  for (vn = GetFirst(); vn; vn = GetNext())
    ...
    */
    ...
    if (!strcmp(item_type, NEPH_STR))
      {
      add_name_to_RAWTBL("XP10");
      add_name_to_RAWTBL("XT10");
      add_name_to_RAWTBL("XRH10");
      add_name_to_RAWTBL("XTSG10");
      }
    else
    if (!strcmp(item_type, DPRES_STR))
      {
      rate = 5;
      add_name_to_RAWTBL("PSFRD");
      add_derived_names("PSFRD");
      }
    else
    if (!strcmp(item_type, RDMA_STR))
      {
      add_name_to_RAWTBL("RDMAVD");
      add_name_to_RAWTBL("RDMAVSH");
      add_name_to_RAWTBL("RDMAQA");
      add_name_to_RAWTBL("RDMAQSH");
      add_name_to_RAWTBL("RDMAQS");
      add_name_to_RAWTBL("RDMAQEX");
      add_name_to_RAWTBL("RDMAPABS");
      add_name_to_RAWTBL("RDMARH");
      add_name_to_RAWTBL("RDMATEMP");
      add_name_to_RAWTBL("RDMAHVPS");
      }
    else
    if (!strcmp(item_type, JPLTDL_STR))
      {
      rate = 1;
      add_name_to_RAWTBL("MRLHS");
      add_name_to_RAWTBL("MRLHW");
      add_name_to_RAWTBL("PSLH");
      add_name_to_RAWTBL("ATLH");
      }
    else
    if (!strcmp(item_type, LHTDL_STR))
      {
      rate = 4;
      add_name_to_RAWTBL("WVVV");
      add_name_to_RAWTBL("WVMR");
      add_name_to_RAWTBL("WVRH");
      add_name_to_RAWTBL("WVPS");
      add_name_to_RAWTBL("WVAT");
      add_name_to_RAWTBL("WVPF");
      add_name_to_RAWTBL("WVPR");
      add_name_to_RAWTBL("WVIX");
      add_name_to_RAWTBL("WVZO");
      }
    else
...
*/    }
    ...
    if (!strcmp(item_type, AEROSOL_STR))  /* Ogren for INDOEX */
      {
      add_name_to_RAWTBL("XBCUT");
      add_name_to_RAWTBL("XBABS");
      add_name_to_RAWTBL("XBSPD");
      add_name_to_RAWTBL("XBSPW");
      add_name_to_RAWTBL("XBSPWRH");
      }
    else
    if (!strcmp(item_type, MASSPEC_STR))  /* Mass-spectrometer for INDOEX */
      {
      add_name_to_RAWTBL("XMASS1");
      add_name_to_RAWTBL("XMASS2");
      add_name_to_RAWTBL("XMASS3");
      add_name_to_RAWTBL("XMASS4");
      }
    else
...
static void initHDR(char vn[])
{
  ...
  add_name_to_RAWTBL("DAY");
  add_name_to_RAWTBL("MONTH");
  add_name_to_RAWTBL("YEAR");
...
*/
  ...
  cp->Average		= cp->type[0] == 'C' ? SumSDI : AverageSDI;
  cp->Dirty		= FALSE;
  cp->Modulo		= NULL;
  cp->Output		= TRUE;
...
/* -------------------------------------------------------------------- */
static void initTans(char vn[])
{
  int		i;
  RAWTBL	*rp;
  char		*names[50], *p;

  ReadTextFile(GPSTANSNAMES, names);

  for (i = 0; names[i]; ++i)
    {
    if (names[i][0] == '#' || strlen(names[i]) < 2)
      continue;

    for (p = strtok(names[i], " \t"); p; p = strtok(NULL, " \t\n"))
      {
      if ((rp = add_name_to_RAWTBL(p)) == (RAWTBL *)ERR)
        continue;
/*
      if (strncmp(rp->name, "GSTAT", 5) != 0 &&
          strncmp(rp->name, "GMODE", 5) != 0)
        rp->DynamicLag = 1;
*/
      }
    }
...
static void initPMS1Dv3(char vn[])
{
   ...
   */
  GetSerialNumber(vn, &p);
  AddToPMS1DprobeList(probe, location, p, pms1_probes[indx].type);
  ...
  if (strncmp(temp, "AS100", 5) == 0)
    ...
    add_name_to_RAWTBL("FREF");
    add_name_to_RAWTBL("FTMP");
    }

  if (strncmp(temp, "AS200", 5) == 0)
    {
    add_name_to_RAWTBL("PHGB");
    add_name_to_RAWTBL("PMGB");
    add_name_to_RAWTBL("PLGB");
    add_name_to_RAWTBL("PFLW");
    add_name_to_RAWTBL("PREF");
    add_name_to_RAWTBL("PFLWS");
    add_name_to_RAWTBL("PTMP");
    }

  if (strncmp(temp, "AS300", 5) == 0)
    {
    add_name_to_RAWTBL("FREF3");
    add_name_to_RAWTBL("FTMP3");
    }
