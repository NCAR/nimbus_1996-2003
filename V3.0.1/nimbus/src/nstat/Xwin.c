...
Widget	inputFileText;

Widget	menuBar, timeDisplayText, list1, goButton;

Widget	ts_text[16], outputText;
...
Widget parent;
{
	...
	Widget		FNframe, fileNameRC, form;
	int			i;
	...
	n = 0;
	form = XmCreateRowColumn(fileNameRC, "fnRC", args, n);

    n = 0;
    XtManageChild(XmCreateLabel(form, "FNlabel0", args, n));

	n = 0;
	inputFileText = XmCreateTextField(form, "inputFileText", args, n);
...
/* -------------------------------------------------------------------- */
Widget CreateEditWindow(parent)
Widget parent;
{
	Arg			args[16];
	Cardinal	n;
	Widget		editForm, ebFrame, ebRC, b[2];
	...
	XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM); n++;
	XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM); n++;
	ebFrame = XmCreateFrame(editForm, "ebFrame", args, n);
	XtManageChild(ebFrame);

	n = 0;
	ebRC = XmCreateRowColumn(ebFrame, "ebRC", args, n);
	XtManageChild(ebRC);


	n = 0;
	b[0] = XmCreatePushButton(ebRC, "quitButton", args, n);
	XtAddCallback(b[0], XmNactivateCallback, Quit, NULL);
	XtManageChild(b[0]);

	n = 0;
	b[1] = XmCreatePushButton(ebRC, "printButton", args, n);
	XtAddCallback(b[1], XmNactivateCallback, PrintText, NULL);
	XtManageChild(b[1]);


	n = 0;
	XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET); n++;
	XtSetArg(args[n], XmNtopWidget, ebFrame); n++;
	XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM); n++;
	XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM); n++;
	XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM); n++;
	outputText = XmCreateScrolledText(editForm, "outputText", args, n);
	XtManageChild(outputText);
