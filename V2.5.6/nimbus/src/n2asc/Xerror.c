...
void CreateErrorBox(Widget parent)
{
	...
	XtSetSensitive(	XmMessageBoxGetChild(errorBox, XmDIALOG_CANCEL_BUTTON),
					FALSE);
	XtSetSensitive(	XmMessageBoxGetChild(errorBox, XmDIALOG_HELP_BUTTON),
					FALSE);

	XtAddCallback(errorBox, XmNokCallback, (XtCallbackProc)ErrorOK,
							(XtPointer)NULL);
