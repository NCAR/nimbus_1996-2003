  ...
  for (vn = GetFirst(); vn; vn = GetNext())
    ...
    */
    ...
    if (!strcmp(item_type, DPRES_STR))
      ...
      add_derived_names("XPSFD");
      }
    else
    if (!strcmp(item_type, JPLTDL_STR))
      {
      rate = 1;
      add_name_to_RAWTBL("TDLMRS");
      add_name_to_RAWTBL("TDLMRW");
      add_name_to_RAWTBL("TDLPS");
      add_name_to_RAWTBL("TDLAT");
      }
    else
...
static void initMASP(char vn[])
{
  ...
  if ((indx = locatePMS(vn, pms1v2_probes)) == ERR)
    {
    sprintf(buffer, "Unknown MASP probe: %s, continuing\n", vn);
    LogMessage(buffer);
    return;
    }
...
static DERTBL *add_name_to_DERTBL(char name[])
{
  ...
  if (LinearSearchTable((char **)derived, nderive, name) != ERR)
    {
    sprintf(buffer, "%s already added to derived list, ignoring duplicate, check dependencies.\n", name);
    LogMessage(buffer);
    return((DERTBL *)ERR);
    }
