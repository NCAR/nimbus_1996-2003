...
static int	FIRST_BIN[MAX_200Y], LAST_BIN[MAX_200Y], SampleRate;
...
void c200yInit(RAWTBL *varp)
{
  ...
  ComputePMS1DParams(radius, esw, dia, minRange, resolution, nDiodes, varp->Length);
...
DERTBL	*varp;
{
  ...
  for (i = FIRST_BIN[probeNum]; i < LAST_BIN[probeNum]; ++i)
    ...
    sampleVolume[i] = tas * (dof[i] * esw[i]);
    }
