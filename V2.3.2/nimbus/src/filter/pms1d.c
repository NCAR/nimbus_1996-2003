...
/* -------------------------------------------------------------------- */
void WritePMS1DdataToSHMEM(data)
NR_TYPE	data[];
{
	int	i;

	for (i = 0; i < *nPMS1Dprobes; ++i)
		{
/*		if (PMS1Dprobe[i].active == 0)
			continue;
*/
...
/* -------------------------------------------------------------------- */
void AddToPMS1DprobeList(probe, location, serial_num, type)
char	probe[];
char	location[];
char	serial_num[];
int		type;
{
	strcpy((probe_name[pcnt] = GetMemory(NAMELEN)), probe);
	strcat(probe_name[pcnt], location);
	strcpy((serialNums[pcnt] = GetMemory(NAMELEN)), serial_num);
...
/* -------------------------------------------------------------------- */
float *SetPMS1DnetCDF(name, length)
char	name[];
int		length;
{
	int		indx;
	...
	if (indx == pcnt)
		...
		if (memcmp(&name[1], "FSSP", 4) == 0)
			...
			strcat(PMS1Dprobe[pcnt].rangeName,
					strchr(name, '_'));
			}
		else
