...
void	VerifyDefault(), MarkDirty(), ResetDefaults();
static void	set_defaultText();
...
void SaveDefaults(FILE *fp)	/* Save modified defaults into "Setup" file		*/
{
	...
	for (i = 0; i < nDefaults; ++i)
		if (Defaults[i]->Dirty)
			{
			fprintf(fp, "DEFAULT=%s %d",	Defaults[i]->Name,
											Defaults[i]->nValues);
...
/* ARGSUSED */
void
MarkDirty(w, indx, call)
Widget		w;
int			indx;
XtPointer	call;
{
	Defaults[indx]->Dirty = TRUE;
...
/* -------------------------------------------------------------------- */
static void set_defaultText(indx)
int		indx;
{
	int		i;
	char	temp[32];
