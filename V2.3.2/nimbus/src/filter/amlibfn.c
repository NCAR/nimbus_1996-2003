...
static int	nIniters = 0;
static void	(*Initializer[256])();
static char	*suffix;
...
SearchDERIVEFTNS(char target[])
{
	...
	do
		...
		if ((rc = strcmp(target, deriveftns[mid].name)) == 0)
			{
			if (deriveftns[mid].constructor)
				Initializer[nIniters++] = deriveftns[mid].constructor;
