...
SetXdialog()
{					/* Needs widgets*/
	sprintf(buffer, "%e", xmin);
	XmTextFieldSetString(tw[6], buffer);
	sprintf(buffer, "%e", xmax);
...
SetYdialog()
{
	...
	XmTextFieldSetString(tw[8], buffer);
	sprintf(buffer, "%e", ymax);
	XmTextFieldSetString(tw[9], buffer);
...
XtPointer	callData;
{
	...
	p = XmTextFieldGetString(tw[8]);
	ymin = atof(p);
	XtFree(p);

	p = XmTextFieldGetString(tw[9]);
	ymax = atof(p);
	XtFree(p);
