...
#define UPFCTR		0.999444
#define FCTRF		0.997
#define TAUP		100.0
#define RAD			(M_PI / 180.0)
#define CDM			111120.0
#define ROLL_MAX	11.0
#define NCF			3
...
DERTBL	*varp;		/* Information about current variable	*/
{
	...
	static int		countr = 0, gps = 0, gps_is_flat = 0;
	...
	if (FeedBack == LOW_RATE_FEEDBACK)
		 ...
		 */
		if (gstat > 0 || fabs(glat) > 90.0 || fabs(glon) > 180.0
			|| fabs(roll) > ROLL_MAX)
			gps = 0;
...
label546:
	...
	lonc[FeedBack] += vewc[FeedBack] / (CDM * cos(alat * RAD));
