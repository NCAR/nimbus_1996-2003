...
static NR_TYPE	CVTBL, CVTBR, CVOFF, CVDIV, cvs7;

static NR_TYPE	TempCorrection();
...
void cviInit()
{
	CVTBL	= (GetDefaultsValue("CVTBL"))[0];
	CVTBR	= (GetDefaultsValue("CVTBR"))[0];
	CVOFF	= (GetDefaultsValue("CVOFF"))[0];
	CVDIV	= (GetDefaultsValue("CVDIV"))[0];
...
DERTBL	*varp;
{
	...
	PutSample(varp, 216.68 * esubt(cvdp) / (cvtemp + 273.16));

}	/* END SRHCV */

/* -------------------------------------------------------------------- */
static NR_TYPE TempCorrection(cv, pcn, cvtemp)
NR_TYPE	cv;
NR_TYPE	pcn;
NR_TYPE	cvtemp;
{
