...
void xlhgm232(RAWTBL *varp, void *p, NR_TYPE *output)
{
	...
	for (i = 0; i < varp->SampleRate; ++i)
		{
		status = input[i * varp->ADSoffset] & 0x0001;
		sample = input[i * varp->ADSoffset] & 0xFFFE;
		sample = (sample - varp->convertOffset) * varp->convertFactor
			* FTMTR;

		if (status)
			output[i] = lastGood;
		else
			output[i] = lastGood = sample;
		}
...
void xlhgm232s(RAWTBL *varp, void *input, NR_TYPE *output)
{
	...
	for (i = 0; i < varp->SampleRate; ++i)
		output[i] = ((short *)input)[i * varp->ADSoffset] & 0x0001;
