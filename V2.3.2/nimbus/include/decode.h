 ...
 * diffreent probes before it needs to be re-coded.
 */
...
#define AVAPSNAMES	"%s/defaults/AVAPS.names"
...
#define HdrBlkTimeToSeconds(hdr)                \
	(((struct Hdr_blk  *)hdr)->sec +        \
	(((struct Hdr_blk  *)hdr)->minute * 60) +   \
	(((struct Hdr_blk  *)hdr)->hour * 3600))
...
void	SetLookupSuffix(char *new_suffix),
	...
	AverageSampledData(),
	ApplyCalCoes(NR_TYPE *record, int which_record),
