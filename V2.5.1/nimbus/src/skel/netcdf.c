...
char	file_name[];
{
	...
	for (i = 0; i < nVariables; ++i)
		...
		if (ncvarid(InputFile, vp->name) != ERR)
			{
			ncattcopy(InputFile, vp->inVarID, "units",
												OutputFile, vp->outVarID);
			ncattcopy(InputFile, vp->inVarID, "long_name",
												OutputFile, vp->outVarID);
			ncattcopy(InputFile, vp->inVarID, "SampledRate",
												OutputFile, vp->outVarID);
			ncattcopy(InputFile, vp->inVarID, "OutputRate",
												OutputFile, vp->outVarID);
			ncattcopy(InputFile, vp->inVarID, "MissingValue",
												OutputFile, vp->outVarID);
			ncattcopy(InputFile, vp->inVarID, "VectorLength",
												OutputFile, vp->outVarID);
			ncattcopy(InputFile, vp->inVarID, "Dependencies",
												OutputFile, vp->outVarID);
			ncattcopy(InputFile, vp->inVarID, "CalibrationCoefficients",
												OutputFile, vp->outVarID);
			}
		else
			{
