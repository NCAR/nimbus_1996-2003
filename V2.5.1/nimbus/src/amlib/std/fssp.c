...
static int	FIRST_BIN[MAX_FSSP], LAST_BIN[MAX_FSSP];
static double	PLWFAC[MAX_FSSP], DBZFAC[MAX_FSSP];
static NR_TYPE	sa[MAX_FSSP], tau1[MAX_FSSP], tau2[MAX_FSSP],
		fssp_csiz[MAX_FSSP][BINS_16*4], DENS[MAX_FSSP];
...
DERTBL	*varp;
{
	...
	NR_TYPE	frange;
	NR_TYPE	vol, sv[16];	/* Sample Volume	*/
	...
	vol = tas * sa[probeNum] * fbmfr * (1.0 - (act * 0.71));
