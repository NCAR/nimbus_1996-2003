...
static NR_TYPE	HDRversion;

NR_TYPE angcr(NR_TYPE, NR_TYPE, float, float);


/* -------------------------------------------------------------------- */
void initAPN159(RAWTBL *varp)
{
	char	*p;
...
void xlhgme(RAWTBL *varp, void *p, NR_TYPE *output)
{
	...
	for (i = 0; i < varp->SampleRate; ++i)
		if (HDRversion < 3.1)
		   {
		   hgme = ~input[i * varp->ADSoffset] * RESOLV14BIT * 0.04;
		   hgme = angcr(hgme, chgme[i], 14.4, 0.0);
		   output[i] = hgme * 84.666667;
		   }
		else
		   {
		   hgme = input[i * varp->ADSoffset] * RESOLV16BIT * 0.04;
		   hgme = angcr(hgme, chgme[i], 14.4, 0.0);
		   output[i] = hgme * 84.666667;
		   }
...
void xlchgme(RAWTBL *varp, void *p, NR_TYPE *output)
{
	...
	for (i = 0; i < varp->SampleRate; ++i)
	    if (HDRversion < 3.1)
		output[i] = chgme[i] = ~input[i * varp->ADSoffset]* RESOLV14BIT;
	    else
		output[i] = chgme[i] = input[i * varp->ADSoffset] * RESOLV16BIT;
