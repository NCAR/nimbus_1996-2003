...
static int		disposMultiRateFilter(), iterateMRFilter();
static void		initCircBuff(), disposCircBuff(), putBuff(),
				readFilters(), readAfilter(), initMultiRateFilter(),
...
void Filter(CircularBuffer *PSCB)
{
	...
	for (i = 0; (rp = raw[i]); ++i)
		...
		if (rp->SampleRate == ProcessingRate || rawFilters[i] == NULL)
			{
			...
			if (task == GET_INPUT)
				...
				if (++intermedHzcnt == L)
					...
					if (currentHz == inputRate - 1)
						...
						diff = (tmpRec[SRstart] -
									inputRec[SRstart+currentHz]) / L;
						}
					else
						diff = (inputRec[SRstart+currentHz+1] -
									inputRec[SRstart+currentHz]) / L;
					}
...
{
	int		i;
	char	*nimbus = getenv("NIMBUS");
	char	*filter[1000];
	double	sum = 0.0;

	sprintf(buffer, "%s/filters/%s", nimbus, file);
	ReadTextFile(buffer, filter);

	daFilt->order = atoi(filter[0]);
	daFilt->aCoef = GetMemory(sizeof(filterType) * daFilt->order);
