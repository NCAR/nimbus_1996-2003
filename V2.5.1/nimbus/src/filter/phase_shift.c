...
NR_TYPE		LinearInterpolate(), PolyInterp();

static void	shift(char name[], int start, int rate, int lag),
			shift_1hz(int start, int lag);

static void interp_regular(NR_TYPE, NR_TYPE *, int, int, int, int);

static void	interp_360_angle(), interp_180_angle(), check_for_wrap();
...
void AddVariableToRAWlagList(RAWTBL *varp)
{
	...
	if (varp->StaticLag != 0)	/* Otherwise IRS will print out.	*/
		LogMessage(buffer);
}
	...
	NR_TYPE			*output)	/* Place to put shifted record	*/
{
	...
	for (i = 0; i < nraw_ps; ++i)
		...
		if (abs((lag = rp->StaticLag + rp->DynamicLag)) > 1000)
			...
			sprintf(buffer, "Max lag of |1000| ms exceeded, setting");
			LogMessage(buffer);
			sprintf(buffer, "to %d ms and continuing.\n", lag);
			LogMessage(buffer);
			}
	...
	int		lag)
{
	...
	if (lag > 0)
		...
		if (ngaps < rate-1)
			memcpy( (char *)next_point,
					(char *)&next_rec[source + ngaps], 2*NR_SIZE);
	...
	if (remainder > JITTER * 2)		/* Then Interpolate	*/
		if (strncmp(name, "THDG", 4) == 0)
			interp_360_angle(prev_point, next_point,
				start, rate, gap_size, remainder);
		else
		if (strncmp(name, "LON", 3) == 0)
			interp_180_angle(prev_point, next_point,
				start, rate, gap_size, remainder);
		else
			interp_regular(prev_point, next_point,
				start, rate, gap_size, remainder);
 ...
 */
static void check_for_wrap(points)
NR_TYPE	points[];
{
	int		i;
