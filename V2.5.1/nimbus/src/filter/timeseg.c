	...
	int				initMode)
{
	...
	if (prevTime >= newTime)
	   ...
	   "  previous time = %02d:%02d:%02d\n       odd time = %02d:%02d:%02d\n",
				h, m, s, ADShdr->hour, ADShdr->minute, ADShdr->sec);
			LogMessage(buffer);

			if (FindNextLogicalRecord(ADSrecord, 122400L) <= 0)
				return(GAP_FOUND);				/* 34:00:00	*/
	...
	/* Short time gap (less than 90 seconds), so fill in records with MISS_VAL
	 */
	if (newTime - prevTime < 90)	/* 90 seconds.	*/
		{
		int		h, m, s;
		...
		sprintf(buffer,
			"Time break of %d seconds ending @ %02d:%02d:%02d, filling in with MISSING_VALUE.\n",
			newTime - prevTime - 1, ADShdr->hour, ADShdr->minute, ADShdr->sec);
		LogMessage(buffer);
	...
	if (newTime <= UserEtim[i] || UserEtim[i] == END_OF_TAPE)
		{
		sprintf(buffer, "Break in time sequence of %d seconds, ",
													newTime - prevTime);
		LogMessage(buffer);
