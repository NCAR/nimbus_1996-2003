...
/* -------------------------------------------------------------------- */
void SaveFlightInfo(FILE *fp)	/* Save modified flight into "Setup" file	*/
{
	int		i;
	char	*p;

	for (i = 0; i < nFlightInfo; ++i)
		if (FlightInfo[i].Dirty)
			{
			p = XmTextFieldGetString(flightText[i]);

			fprintf(fp, "FLIGHT=%s %s",	FlightInfo[i].Name, p);
			fprintf(fp, "\n");
			}
...
/*			strcpy(FlightInfo[i]->newValue, new_value); */
